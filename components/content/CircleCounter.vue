...
export default {
  ...
  methods: {
    ...
    getStrokeWidth(stroke) {
      return (stroke * this.UNITS) / 100;
    },

    // Determine the 'end' angle of the path for the active dashes in degrees.
    activeEnd() {
      if (this.value.split('$').join('') == 0) {
        return 0;
      }
      return (
        360 *
        (this.value.split('$').join('') * this.dashPerc() +
          (this.value.split('$').join('') - 1) * this.spacePerc())
      );
    },
